  ...
  <div>
    ...
    >
      ...
      <template #item.actions="{ item }">
        <!-- Botón de Editar -->
        <v-btn small color="primary" @click="openEditProductDialog(item)">Editar</v-btn>
    ...
    <v-dialog v-model="dialog" max-width="500px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="saveProduct">{{ isEditing ? 'Guardar' : 'Agregar' }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
@media (max-width: 600px) {
  ...
  /* Ajustar el tamaño de las imágenes dentro de la tabla */
  .v-img {
    max-width: 50px;
    max-height: 50px;
  }
