  ...
  <div>
    ...
    >
      <template #item.actions="{ item }">
        ...
        <!-- Botón de Eliminar -->
        <v-btn small color="red" dark @click="deleteUserHandler(item.id)">Eliminar</v-btn>
      </template>
    </v-data-table>

    <!-- Diálogo de agregar/editar usuario -->
    <v-dialog v-model="dialog" max-width="500px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" text @click="saveUser">{{ isEditing ? 'Guardar' : 'Agregar' }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
